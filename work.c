...
/*
	Copia um ficheiro para outro, dando uso à execução de "cat"
*/
	...
	while(1) {
    	...
    	if (nl == 0) continue;
    		...
    		if (buf[0] == '$') {
				...
				if (buf[1] == '|') {
					for (int j = save_start; j < i; j++) {
						if (j == save_start) strcpy(cmd, result[j]);
    			    	else {
    			      		strcat(cmd, " | "); 
    			      		strcat(cmd, result[j]); 
    			    	}
